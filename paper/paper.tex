...
and may earn employment income before retirement:
\begin{align}
...
and $a_{\mathrm{ret}}$ is the age at which employment income ceases.

Federal tax law includes up to 85\% of Social Security benefits in ordinary
income; we set the taxable fraction $\phi = 0.85$. Municipal-bond interest
and Roth withdrawals are excluded from ordinary income. The taxable ordinary
income in year $t$ is therefore
\begin{equation}
  I_t \;=\; w^d_t + \phi\,y^{SS}_t + y^{emp}_t.
  \label{eq:income}
\end{equation}
...
\begin{equation}
  c_t \;=\; w^m_t + w^d_t + w^r_t + y^{SS}_t + y^{emp}_t
           - \Tax(I_t) - \pi_t\,w^d_t,
  \label{eq:consumption}
\end{equation}
where $\pi_t$ is defined in~\eqref{eq:penalty_rate}. Equivalently,
$c_t = w^m_t + (1-\pi_t)\,w^d_t + w^r_t + y^{SS}_t + y^{emp}_t - \Tax(I_t)$.
...
\begin{equation}
  s_{tk} \;=\; \max\!\bigl(0,\; I_t - \Delta - L_k\bigr)
         \;=\; \max\!\bigl(0,\; w^d_t + \phi\,y^{SS}_t + y^{emp}_t - \Delta - L_k\bigr).
  \label{eq:slack_def}
\end{equation}
...
\begin{equation}
  w^d_t - s_{tk} \;\leq\; \Delta + L_k - \phi\,y^{SS}_t - y^{emp}_t,
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item Compute both benchmark consumption levels $C^{\mathrm{mdr}(s)}$ and
...
decisions. Therefore $C^{*(s)} \geq \max(C^{\mathrm{mdr}(s)}, C^{\mathrm{drm}(s)})$
...
\[
  x_{i_d(t)} - x_{i_s(t,k)} \;\leq\; \Delta + L_k - \phi\,y^{SS}_t - y^{emp}_t,
