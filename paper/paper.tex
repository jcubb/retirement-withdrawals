...
\begin{align}
  ...
  \label{eq:emp}
  \
  y^{pen}_t   &= ar{y}^{pen} \cdot \mathbf{1}igl[a_0 + t \geq a_{\mathrm{ret}}igr],
  \label{eq:pen}
\end{align}
where $\bar{y}^{SS}$ is the elected annual Social Security benefit, $a_{SS}$
where $ar{y}^{SS}$ is the elected annual Social Security benefit, $a_{SS}$
is the claiming age, $ar{y}^{emp}$ is annual pre-retirement earned income,
$ar{y}^{pen}$ is the annual pension benefit, and $a_{\mathrm{ret}}$ is the age
...
retirement age; setting $ar{y}^{pen} = 0$ recovers the no-pension baseline.
...
\begin{equation}
  s_{tk} \;=\; \max\!\bigl(0,\; I_t - \Delta - L_k\bigr)
         \;=\; \max\!igl(0,\; w^d_t + \phi\,y^{SS}_t + y^{emp}_t + y^{pen}_t - \Delta - L_kigr).
  \label{eq:slack_def}
\end{equation}
